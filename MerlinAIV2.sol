...
// File contracts/interfaces/IMerlinSwapCallback.sol
...
// File contracts/interfaces/IMerlinSwapFactory.sol
...
// File contracts/interfaces/IMerlinSwapFlashCallback.sol
...
// File contracts/interfaces/IMerlinSwapPool.sol
...
// File contracts/MerlinSwapPool.sol
...
contract MerlinSwapPool is IMerlinSwapPool {
    ...
    /// @notice maximum liquidSum for each point, see points() in IMerlinSwapPool or library Point
    ...
    /// @notice state of pool, see library State or IMerlinSwapPool#state for more infomation
    ...
    /// @notice returns infomation of a point in the pool, see Point library of IMerlinSwapPool#poitns for more information
    ...
    constructor() {
        (address _tokenX, address _tokenY, uint24 _fee, int24 currentPoint, int24 _pointDelta, uint24 _feeChargePercent) = IMerlinSwapFactory(msg.sender).deployPoolParams();
        require(_tokenX < _tokenY, 'x<y');
        require(_pointDelta > 0, 'pd0');
        original = address(this);
        factory = msg.sender;
        swapModuleX2Y = IMerlinSwapFactory(msg.sender).swapX2YModule();
        swapModuleY2X = IMerlinSwapFactory(msg.sender).swapY2XModule();
        liquidityModule = IMerlinSwapFactory(msg.sender).liquidityModule();
        limitOrderModule = IMerlinSwapFactory(msg.sender).limitOrderModule();
        flashModule = IMerlinSwapFactory(msg.sender).flashModule();
    ...
    function collectFeeCharged() external override noDelegateCall lock {
        require(msg.sender == IMerlinSwapFactory(factory).chargeReceiver(), "NR");
